...
const SeekerRequestsList = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      // Filter out any requests with null book or owner data
      const validRequests = data.filter((req) => req.bookId && req.ownerId);
      ...
      {requests.length === 0 ? (
        ...
        <div className="requests-list">
          {requests.map((request) => (
            ...
            >
              <div className="request-header">
                {/* Add null checking when accessing book properties */}
                <h3>{request.bookId?.title || "Untitled Book"}</h3>
                <span className={`status-badge ${request.status}`}>
                  {request.status.charAt(0).toUpperCase() +
                    request.status.slice(1)}
                </span>
              ...
              <div className="request-details">
                <p>
                  {/* Add null checking when accessing owner properties */}
                  <span>Owner:</span> {request.ownerId?.name || "Unknown Owner"}
                </p>
                <p>
                  <span>Request Date:</span> {formatDate(request.createdAt)}
                </p>
          ...
          <div className="modal-content">
            ...
            <div className="contact-details">
              <div className="book-info">
                <h4>Book Details</h4>
                <p>
                  {/* Add null checking for contact info */}
                  <span>Title:</span> {contactInfo.book?.title || "Untitled"}
                </p>
                <p>
                  <span>Author:</span>{" "}
                  {contactInfo.book?.author || "Unknown Author"}
                </p>
              </div>
              ...
              <div className="user-info">
                ...
                <p>
                  <span>Name:</span>{" "}
                  {contactInfo.contactInfo?.name || "Unknown"}
                </p>
                <p>
                  <span>Email:</span>{" "}
                  {contactInfo.contactInfo?.email || "No email provided"}
                </p>
                {contactInfo.contactInfo?.mobile && (
                  <p>
                    <span>Phone:</span> {contactInfo.contactInfo.mobile}
